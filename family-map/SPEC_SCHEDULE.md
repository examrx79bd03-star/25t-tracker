...
> **2026-07-05 実装同期（TimeTree 準拠へ再構成）**：各行の**カード枠（背景/ボーダー/角丸）を撤去**し、フラットな行リスト＋ヘアライン区切り（最終行のみ区切りなし）に変更。**タイトルの直後に繰り返し(⟳)・通知(🔔)・外部カレンダー由来(↗) のアイコンをインライン表示**（設定がある時だけ・アイコン欄自体は非表示）——旧実装は外部カレンダーのみ「↗ カレンダー名」という別行のリンクテキストで表示し、繰り返し/通知のインジケータは日リストに存在しなかった。参加メンバーのアバターは行の右端に縦中央寄せ（旧実装は本文プレビューの下に別行でスタック）。ラベル色バー（`border-left`）は維持。
...
> **2026-07-02 実装同期（TimeTree 準拠へ再構成）**：`#evDetailBg` は上から
> ←戻る/⋯メニュー → アバター帯（参加メンバー、無ければ作成者）→ ラベル色タイトル
> （`--ev-accent`＝外部はソース色→ラベル色→カレンダー色）→ 日付2カラム → 本文（左寄せ）
> → チェックリスト → アイコン行（通知[常時表示・なし時は「通知なし」]／繰り返し／
> カレンダー[外部は「編集不可」注記を内包]／ラベル／URL＋OGPカード）→
> 活動履歴とコメントを日付区切りで時系列統合した単一フィード（`renderEventFeed`）→
> 下部 sticky コメントバー（丸ピル入力＋↑送信）。
> 旧「活動履歴」「コメント」2セクションと下部 削除/閉じる/編集 ボタン行は撤去
> （編集・削除は⋯メニューへ。外部イベントでは⋯内の編集/削除を非表示）。
...
> **2026-07-03 実装同期（TimeTree 準拠へ再構成）**：`#evEditorBg` は
> ✕（左上）／保存（右上・白ピル、既存の dynamic header button）→
> 大きなボーダーレスのタイトル入力（placeholder「タイトル」）→
> ヘアライン区切りのアイコン行スタック：カレンダー行（📅+色ドット+名前+›）／
> 終日トグル行＋開始・終了（右寄せのグレーチップ型 date/time input、
> 終日ONで時刻チップ非表示）／ラベル行（タグアイコン+色バー+名前+›→
> **ボトムシート単一選択ピッカー `#evLabelPickerBg`**、旧インラインチップは廃止）／
> メンバー行（人アイコン+アバター+›）／通知行（時計アイコン+値、
> 設定時のみ ⓧ 解除ボタン）→ **＋チップ行**（繰り返し[チップ文言が現在値に変化]・
> URL・メモ・チェックリスト＝タップでセクションを表示、値があれば最初から表示）。
> 下部の キャンセル/保存 ボタンは撤去（削除のみ編集時に残置）。
> 「メモに保存する」トグルは 2026-05-27 のぐっち要望どおり**非表示を維持**
> （ベンチマークには映っているが、意図的な family-map の差分）。
>
> **2026-08-11 実装同期**：本文（`#evBodyInput`）は**打った行数に合わせて縦に伸びる**
> （10 行で頭打ち→スクロール）。上限は JS の `BODY_MAX_ROWS` と CSS の
> `.textarea { max-height }` に二重で持つので、**片方を変えたらもう片方も直す**。
> 作法と地雷は cookbook `03-browser-html/auto-grow-textarea.md` が正。
...
> **2026-07-29 実装同期（選べる色を 12 → 72 に拡張）**：上の 6 色は**デフォルトラベルの初期値**として現在も正（`DEFAULT_LABELS`）。変わったのは**ピッカーで選べる色**のほうで、「北欧系トンマナに限定」の方針はユーザー判断で撤回した。ラベル・カレンダー・iCloud カレンダー・メンバーアバターの 4 系統すべてが**共通の 72 色パレット `COLOR_PALETTE`（16 色相 × 4 トーン ＝ 64 色 ＋ 彩度 0 の白→黒 8 段）**を参照する（旧 `LABEL_PALETTE` / `MEMBER_BG_PALETTE` はその別名）。**旧 12 色は全部 64 色の中に残存**＝既存の選択は無効化されない。**色相色の明度には上下限がある**（既存で最も明るいマスタード `#d4a14a` より明るくしない）——色は白文字の下地と、クリーム地の上の文字色の**両方**に使われるため。**無彩色ランプだけはその枠外**なので、`readableTextOn()` / `readableInk()` が下地の明るさに応じて文字色を反転・減光して両用途を成立させている（しきい値は既存色が 1 つも掛からない 0.42）。詳細は index.html の `COLOR_PALETTE` / `LIGHT_CUTOFF` 定義コメントを正とする。
